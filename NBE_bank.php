...
<div class="col-md-8">
        ...
        border-radius: 20px 20px 20px 20px;">
            ...
            <div class="media-body">
              <h2 class="text-dark display-12" style="font-family:Century Gothic;" >NBE Bank
    ...
    <p style="background-color:#55608f;font-weight:bold;font-size:16px;color:white;padding:2%;">Here you can Find NBE Bank Report per Day</p>
    </aside>
  </div>
</center>
<center>
...
SELECT distinct requestid
  ...
  --and datediff(day,creation_time,getdate())<>0
   and ([nodeID] like '%NBE%' and [nodeID] not like '%BMW%' and[nodeID] not like '%IBM%' ))
  t
  pivot (
  count (requestid)
  FOR ticket_group IN (
...
SELECT distinct [KPI_Status_RawData].requestid
  ...
  and datediff(day,creation_time,getdate())<>0 and ([KPI_Status_RawData].[nodeID] like '%NBE%' and [KPI_Status_RawData].[nodeID] not like '%BMW%' and [KPI_Status_RawData].[nodeID] not like '%IBM%' ))
  ...
  and ([KPI_Status_RawData].[nodeID] like '%NBE%' and [KPI_Status_RawData].[nodeID] not like '%BMW%' and [KPI_Status_RawData].[nodeID] not like '%IBM%')  ");
...
if($output_query["last_update"] == NULL ){
