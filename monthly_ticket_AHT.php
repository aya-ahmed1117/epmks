...
include ("pages.php");
      ?>
<head>
      <title>Monthly ticket AHT</title>
...
<div class="col-md-8">
        ...
        border-radius: 20px 20px 20px 20px;">
            ...
            <div class="media-body">
              <h2 class="text-dark display-12" style="font-family:Century Gothic;" >Ticket AHT Monthly
...
group by RequestID,[username]),

AHT_Details as
(select datepart(MONTH,([final_close])) [month]
,[username]
,AHT1.RequestID
...
where ((Datepart(MONTH,[Final_close])=DATEPART(MONTH, DATEADD(MONTH, -1, getdate()))AND DATEPART(yyyy, [Final_close]) = DATEPART(yyyy, DATEADD(MONTH, -1, getdate())) )or 
(Datepart(MONTH,[Final_close])=DATEPART(MONTH,getdate()) AND DATEPART(yyyy, [Final_close]) = DATEPART(yyyy,getdate())) )
...
select *
from AHT_Details where username in ( select username from [Aya_Web_APP].dbo.employee_web_table where manager='$self')");
...
<script type="text/javascript">
        function Export() {
            $("#tblCustomers").table2excel({
                filename: "monthly_ticket_AHT.xls"
            });
        }
    </script>
