...
<div id="checkboxDiv" style="display:none;">
  <input type="checkbox" id="checkbox"> I confirm my leave request
  <a href="#">HR rules</a>
</div>

<input type="submit" id="submit" class="btn btn-primary submit" onclick="compare()"
  ...
  $(document).ready(function(){
    $('#inputGroupSelect01').on('change', function(){
      ...
      if(this.value == "Annual Leave"){
        ...
        alert("You have selected Annual Leave.");
        
        // Hide stop button and display checkbox
        $('#stop').hide();
        $('#checkboxDiv').show(); // Show checkbox for confirmation
        
        // If checkbox is checked, show submit button
        ...
        // Show stop button when not Annual Leave
        $('#stop').show();
        $('#checkboxDiv').hide(); // Hide checkbox
        $('#submit').show(); // Always show submit button when not Annual Leave
      }

      
    });

    // When the checkbox is checked or unchecked, show or hide submit button
    $('#checkbox').on('change', function(){
      if($(this).is(':checked')) {
        $('#submit').show(); // Show submit button when checkbox is checked
      } else {
        $('#submit').hide(); // Hide submit button if checkbox is unchecked
      }
    });
  });
</script>
