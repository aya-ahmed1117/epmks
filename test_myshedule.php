...
<?php if ($_SESSION['role_id'] == 0){
    ...
    $check_user_sql = sqlsrv_query( $con ,"SELECT * FROM new_sch_view 
        WHERE  username = '$s_username'");

          $count_results  = 1 ;
              
            while($echo = sqlsrv_fetch_array($check_user_sql)){
                $schedule_dates = $echo['schedule_date']->format('Y-m-d');
                $shift_starts =$echo['shift_start'];
      }
   if($count_results != 0){
...
$week .= str_repeat('<td style="height: 50px;background-color:lightgray;" name="schedule_date"><a href="?schedule_dates='.$schedule_dates.'"></td></a>', $str);
  ...
  if(!empty($schedule_date)){
      ...
      $this_day = date('Y-m-d');
        if($this_day == $schedule_date){
            ...
            <p style="color:gray;">('.$shift_start.')</p></td>';
        }
     else{    
        if(($shift_start == 'off') || ($shift_start == 'OFF')){
            $week .= '<td class="hovers">'.$calenderDay.'<p style="color:red;">
            ('.$shift_start.')</p></td>';
       ...
       if(
        ...
        $week .= '<td
            class="hovers">'.$calenderDay.'<p style="color:gray;">('.$shift_start.')</p></td>';
         }
        } /// end !empty
    }
    if(empty($schedule_date)){
        $week .= '<td class="hovers"><p style="color:blue;">No Schedule</p></td>';
      }  
