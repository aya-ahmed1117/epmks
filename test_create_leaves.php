...
$output_queryshift = sqlsrv_fetch_array($qryshift);
   ...
   $resshift_end = $output_queryshift['shift_end'];
  echo $resultss = $output_queryshift['result'];

// $qryshift =  sqlsrv_query( $con ,"SELECT [shift_start],[shift_end]
//   ,IIF(([Sch_tbl].[shift_start] between '12:00:00' and '23:59:59' and (cast(cast([Sch_tbl].[shift_end] as datetime)-cast([Sch_tbl].[shift_start] as datetime) as time) ='12:00:00') or ([Sch_tbl].[shift_start] between '16:00:00' 
//   and '23:59:59'))
//          ,[open]
//          ,[error])
//   FROM [Aya_Web_APP].[dbo].[schedule_table]
...
if($interval <= 0 ) {
  echo '<div id="messagess5s" class="overlay">
  <div class="popup">
    <h2> Sorry last '.$s_username.'</h2>
    <span class="deleteMeetingClose">&times;</span>
<br/>
...
<script type="text/javascript">
  (function() {
  document.getElementById("messagess5s").style.display = "none";
});
...
<?php 
// if($resultss !== 'open' && $type == "Official Mission"){
//   //open
//  // echo 'hena fe moshkela';//}
// }
?>
<script type="text/javascript">
...
/////////
  ...
  if( (new Date(startDt).getTime() != new Date(endDt).getTime())  && 
    (ptype == "Permission") || (ptype == "Official Mission" ) && (test == 'error'))
  {
