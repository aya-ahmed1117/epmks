...
$first_query = sqlsrv_query( $con ,"SELECT * FROM leaves WHERE 
	username ='$s_username' and [type] = '$type' and year([adate]) >='$CurrentYear'  order by [creation_time] DESC ");

while($output_query = sqlsrv_fetch_array($first_query)){
$rows .='<tr >';
$rows .='<td class="sorting_1">'.$output_query["id"].'</td>';
$rows .='<td class="sorting_1">'.$output_query["type"].'</td>';
...
$rows .='<td class="sorting_1">'.$output_query["wfm_note"].'</td>';
