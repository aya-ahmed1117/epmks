...
.ui-datepicker .ui-datepicker-title {
 ...
 <?php 
  if($role_id >1){
    $new_querys= sqlsrv_query( $con ,"with employeelist as (SELECT 
  a.username ,manager
  from [Aya_Web_APP].dbo.employee_web_table a
  left join Employess_DB.dbo.tbl_Personal_info  b on a.username = b.UserName
  left join Employess_DB.dbo.Tbl_SubGroups on SubGroup_ID = sub_Group
  
  where unit = 12 and ([group] <> '21' or [group] is null )  and ( SubGroups not in ('Mail','Phone','TAM','GDS') or SubGroups is null) and grade = 'L8'
  ...
  where unit = 12 and [group] = '21' and grade = 'L8'
  ...
  SELECT ISNULL((
  SELECT top 1 username from employeelist
  where manager = '$self' ),'404') username");
...
<div id="message" class="overlay">
    <div class="popup">
      ...
      <span class="error-code-bg text-bold" style="text-transform: uppercase;"><br> <i class="fa fa-exclamation-triangle"></i>Sorry, you don`t have any employee to edit or create sch for him</span>
        </div>
    </div>';
}}
                ?>
  ...
  <form method="post"  onsubmit="return compare()">
    ...
    <div class="col-lg-6">
      <div class="input-group mb-5">
        ...
        <input type="date" class="form-control" placeholder="From Date" aria-label="From Date" id="adate" min="<?php $tomorrow 
        = date("Y-m-d", strtotime("1 day"));echo $tomorrow; ?>" 
      name='date' aria-describedby="basic-addon1"
      value='<?php if(isset($_POST['date'])) echo $_POST['date']; ?>' required onkeydown="return false" onpaste="return false"/>
      </div>
    </div>
<br>
    <div class="col-lg-6">
      <div class="input-group mb-5">
        ...
        <input type="date" class="form-control" placeholder="To Date" aria-label="To Date" name="date2" id="bdate" min="<?php $tomorrow 
        = date("Y-m-d", strtotime("1 day"));
                  echo $tomorrow; ?>"
          aria-describedby="basic-addon1" required onkeydown="return false" onpaste="return false"
...
$checks = sqlsrv_query( $con ,"with employeelist as (SELECT 
  a.username ,manager
  from [Aya_Web_APP].dbo.employee_web_table a
  left join Employess_DB.dbo.tbl_Personal_info  b on a.username = b.UserName
  left join Employess_DB.dbo.Tbl_SubGroups on SubGroup_ID = sub_Group
  
  where unit = 12 and ([group] <> '21' or [group] is null )  and ( SubGroups not in ('Mail','Phone','TAM','GDS') or SubGroups is null)
  ...
  select username from employeelist
  where manager ='$self' ");
  ...
  username from [Aya_Web_APP].[dbo].[employee] where role_id = 0 order by username
   ");
  while($outputs = sqlsrv_fetch_array($checks)){
    $usernames = $outputs['username'];
        $rows = '<option ';
...
if(isset($_POST['days'])){$days = $_POST['days'];}
if($usernames <> '0'){
     $user_query = sqlsrv_query( $con ,"EXEC Auto_sch_update_edit 
      @user_name='$usernames' 
, @Start_time='$myTimes2'
, @end_time='$myTimes5'
, @Date_from='$mydate'
