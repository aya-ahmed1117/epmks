...
if($type == 'Permission'){
  ...
  echo$CurrentMonth = date("m",strtotime($adate)).'this month<br>';
...
if (date("d",strtotime($adate)) < 14 ) {
  ...
  echo $NextYear = date("Y", strtotime($adate)).'this year<br>';
  echo $NextMonth = date("m", strtotime($adate)).'next mounth<br>';
}
else
{
  $CurrentYear = date("Y",strtotime($adate));
...
<br/>
    ...
    echo "SELECT sum([count]) as countUnits
...
{
//insert query +  units 
  $time_query = "INSERT INTO leaves ([username] , [engineer_id] , [adate] , [bdate] , [starttime] , [endtime] , [type],[notes],[count] , [status] , [creation_time]) VALUES ('$s_username','$engineer_id','$adate','$bdate','$starttime','$endtime','$type','$notes','$units' , '$status' , '$sqltime' )";//);
