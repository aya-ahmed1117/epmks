...
if($type == 'Annual Leave'){
 ...
 WITH a as ( 
  ...
  left join Annuel_cal on Annuel_cal.Username = p.UserName 
  where [Employee_Status] <>'Resigned' and 
  [Employee_Type]='OutSource' and
  (DATEDIFF(DAY, [Hiring_Date], '$adate') ) >= 180
  ) 
  SELECT 
  ISNULL( (select username from a
  where username =  '$s_username'  ) , 'Eligible') result */
...
v*/
 ...
 if ($results == 'Not Eligible'){
  echo '<script>
    swal({
    title: "You have not completed the first six months, so a leave request cannot be submitted until they are completed. Thank you.",
...
$('#countDays').val(Math.round(days));
  ...
  <script type="text/javascript">
   /* // Bind the change event to the select element
    $('#inputGroupSelect01').on('change', function() {
      if ($(this).val() === "Annual Leave" && 
        <?php echo $results != 'staff' ? 'true' : 'false'; ?>) {
        swal({
          title: "You have not completed the first six months, so a leave request cannot be submitted until they are completed. Thank you.",
          icon: "warning",
        });
...
<!-- <script type="text/javascript">
  ($('#inputGroupSelect01').onChange = val("Annual Leave") && 
    <?php echo $results != 'staff';?>)
  {
    swal({
    title: "You have not completed the first six months, so a leave request cannot be submitted until they are completed. Thank you.",
    icon: "warning",})
