...
while ( $output_engineers = sqlsrv_fetch_array($check_engineers)){
  $schedule_date = $output_engineers['schedule_date']->format('Y-m-d');
  $shift_start =$output_engineers['shift_start'];
  $shift_ends =$output_engineers['shift_end'];
}
//if($schedule_date <> NULL){

    if(!empty($schedule_date)){
if(($shift_ends == '') || ($shift_start == NULL)){
$this_day = date('Y-m-d');
  //$week .= '<td class="hovers"  >'.$calenderDay.'';
    if($this_day == $schedule_date){
        ...
        class="today hovers" >Today
        <p style="color:gray;" title="'.$shift_ends.'">('.$shift_start.')</p>
        <span style="font-siza:5px; color:darkblue;">'.$shift_ends.'</span></td>';
    }
}else{
    
    if(($shift_start == 'off') || ($shift_start == 'OFF')){

$week .= '<td class="hovers" title="'.$shift_ends.'">'.$calenderDay.'<p style="color:orange;">('.$shift_start.')</p></td>';
    ...
    //$week .= '<p style="color:orange;">'.$shift_start.'</p>';
       ...
       if($shift_start == 'Annual Leave'){
        ...
        class="hovers"title="'.$shift_end.'">'.$calenderDay.'<p >('.$shift_start.')</p>
        </td>';
        }
        ...
        class="hovers">'.$calenderDay.'<p>('.$shift_start.')</p>
         ...
         if($shift_start == 'Sick Leave'){
            $week .= '<td style="height: 50px;border:2px solid #fff;background-color:#ffc107;color:black;"        class="hovers">'.$calenderDay.'<p>('.$shift_start.')</p>
        </td>';
        }
/*    
 if($shift_start=='OFF'){
$week .= '<p style="background-color:#198754;color:#eee;">
('.$shift_start.')</p>';}
*/
    ...
    $week .= '<td
        class="hovers"title="'.$shift_end.'">'.$calenderDay.'<p style="color:gray;">('.$shift_start.')</p>
