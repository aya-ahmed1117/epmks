...
<h5 id="myDIV" style="float: right;font-size:20px;color:white;font-style:initial;white-space: pre;font-weight: bolder;line-height:inherit;">  View old ticket .. </h5></a>  
  ...
  <div class="input-group sm-5" id="Last_working_date"
   title="Please Update the Date or it Will be Automatic For Today"> 
    ...
    <input type="date" min="<?php $tomorrow 
        = date("Y-m-d", strtotime("0 day"));
          echo $tomorrow; ?>" class="form-control cur_date" 
          name="Last_working_date" placeholder="Choose date"  id="adate"
      aria-describedby="basic-addon1" 
      value="<?php 
                    // Check if Last_working_date is set or null, if null set to current date
                    echo !empty($Last_working_date) ? $Last_working_date : date('Y-m-d'); 
                ?>"
                />
  </div>
...
$checks = sqlsrv_query( $con ,"SELECT * FROM employee
  ...
  employee.username
  WHERE role_id = 0 and Employee_Status <>'Resigned'  order by employee.username  ");
...
$checks = sqlsrv_query( $con ,"SELECT * from  employee where role_id != 1 and  manager_id = '$aya' and username <> '$s_username' order by username ");
...
$checks = sqlsrv_query( $con ,"SELECT * from  employee where role_id != 1 
  and [Unit_Name] ='$unit' and username <> '$s_username'  order by username   ");
  while($outputs = sqlsrv_fetch_array($checks)){
  $rows = '<option ';
    $rows .= $output['id'] == $outputs['id'] ? "selected" : "";;
  $rows .= ' value="'.$outputs['username'].'">'.$outputs['username'].'</option>';
...
<div  class="input-group" id="sub_group">
  ...
  <select class="form-control" 
  name="sub_group">
      <option action="none" value="" selected>Select sub group</option>
...
$checks = sqlsrv_query( $con ,"SELECT * from  employee where role_id  not in (1,0,7) and username not in ('y_test','Z_test') and [Unit_Name] ='$unit' and username <> '$s_username' order by username");
  while($outputs = sqlsrv_fetch_array($checks)){
  $rows = '<option ';
    $rows .= $output['id'] == $outputs['id'] ? "selected" : "";;
  $rows .= 'value="'.$outputs['username'].'">'.$outputs['username'].'</option>';
...
<div id="checkboxDiv" style="display:none;">
  <input type="checkbox" id="checkbox"> <samp style="font-size:20px;font-weight: bold;">Please click on checkbox after filling the </samp>
  <a href="https://forms.office.com/r/Zz0H5N1FCa" style="color:orangered;font-weight: bold; text-decoration: underline; " target="_blank">HR exist interview form</a>
</div>
...
$Creation_time      = date ("Y-m-d H:i:s");
  $promotion        = sqlsrv_query($con1 ,"SELECT case 
     when Grade='L8' and '$Employee_grade'='L7' then 'ok' 
...
where UserName='$Employee_Username'"); 
  //while( $outputs = sqlsrv_fetch_array($promotion)){
    ...
    and [Request_status] ='closed' ", array(), array( "Scrollable" => 'static' ));
    ...
    $insert_promot =sqlsrv_query($con,"INSERT INTO 
      [Aya_Web_APP].[dbo].[tbl_Ticketing_system] 
...
$insert_query = sqlsrv_query( $con ,"INSERT INTO [Aya_Web_APP].[dbo].[tbl_Ticketing_system] 
   ( [Request_Type],[Requester_username],[Note]
      ...
      ,[Reason_of_leave] , [Employee_grade] , [Employee_app_Id] , [sub_group])

  VALUES ( '$Request_Type', '$Requester_username', '$Note' , '$Request_status', '$Creation_time', '$Ticket_Subject',
    '$Employee_Username','$Last_working_date','$Employee_new_manager','$Employee_new_id','$Employee_new_username'
    , '$Reason_of_leave' , '$Employee_grade' , '$engineer_id' , '$sub_group')");
