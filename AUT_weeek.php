...
<head>
      <title>Absens/utiliz/task week</title>
...
<div class="col-md-8">
        ...
        border-radius: 20px 20px 20px 20px;">
            ...
            <div class="media-body">
              <h2 class="text-dark display-12" style="font-family:Century Gothic;" >Absenteeism / utilization / Task's Weekly
...
WHERE  [schedule_table].[shift_start] <> 'off' and cast([schedule_table].schedule_date as date) between 
(SELECT cast(DATEADD(wk, DATEDIFF(wk, 6, '1/1/' + cast(year(getdate())as nvarchar)) + (datepart(week,getdate())-2), 6)as date)) and dateadd(day,6,cast(DATEADD(wk, DATEDIFF(wk, 6, '1/1/' + cast(year(getdate())as nvarchar)) + (datepart(week,getdate())-2), 6)as date))
...
--,select * from tbl_leave_deduc where username ='ammar.hussien'
...
left join [Aya_Web_APP].dbo.employee_web_table a on a.username = Final_absec.username
...
$rows = '<tr>';
  ...
  $rows .='<td class="hovers" style="border: 1px solid lightgray;background-color:#6b5b95;color:white;">'.floor(($echo['Absenteeism'])*100).'%'.'</td>';
  //
  ...
  $rows .='<td class="hovers" style="border: 1px solid lightgray;background-color:#6b5b95;color:white;">'.floor(($echo['Tasks'])*100).'%'.'</td>';}		
$rows .= '</tr>';
echo $rows;
