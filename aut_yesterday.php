...
<head>
       <title>Absens/utiliz/task yesterday</title>
...
<div class="col-md-8">
        ...
        border-radius: 20px 20px 20px 20px;">
            ...
            <div class="media-body">
              <h2 class="text-dark display-12" style="font-family:Century Gothic;" >Absenteeism / utilization / Task's Yesterday
...
WHERE  [schedule_table].[shift_start] <> 'off' and cast([schedule_table].schedule_date as date) = cast(dateadd(day,-2,getdate()) as date))
...
,sum(cast(cast([work_duration] as datetime) as float)) [work_duration]
       ...
       where cast([date] as date) = cast(dateadd(day,-2,getdate()) as date)
...
where Final_absec.username  in ( select username from [Aya_Web_APP].dbo.employee_web_table where manager = '$self') 
order by 1,2,3
...
");
  ...
  $rows = '<tr>';
   if($echo["Sch_day"] == NULL ){
    $rows .='<td class="hovers" style="border: 1px solid lightgray;background-color:lightgray ;">
Blank</td>';
  }else{
  $rows .='<td class="hovers" style="border: 1px solid lightgray;">'.$echo['Sch_day']->format('Y-m-d').'</td>';}
...
<script type="text/javascript">
        function Export() {
            $("#tblCustomers").table2excel({
                filename: "Absen_utiliz_Tas_yesterday.xls"
            });
        }
    </script>
