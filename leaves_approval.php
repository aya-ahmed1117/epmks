...
($Unit_Name == 'Onsite Problem Management') ){// unit
...
////if($Unit_Name == 'Enterprise Service Desk'){
    if(($Unit_Name == 'Enterprise Service Desk') || 
      ($Unit_Name == 'Onsite Problem Management') ){
    if($mydate <= $ysterday2){
...
if($output_query["creation_time"] == null){
...
//  Note</button></td>';
 ...
 $rows .= '<td><input type="button" name="view" value="Notes" id="'.$output_query["id"].'" class="btn btn-info btn-xs view_data" data-type="'.$output_query["type"].'"/></td>';
...
<div id="dataModal" tabindex="-1" role="dialog" >
     ...
     <div class="modal" id="message21" >
      <div id="myOut" class="modal-content" >
        ...
        <h5 class="modal-title" >Leaves id num</h5>
         ...
         <input type="text" class="form-control type"  disabled="true" />
         <?php //echo  ?>
 ...
 <script>  
     ...
     $(document).on('click', '.view_data', function(){  
           ...
           var wfm_note = $('.wfm_note').val(); 
        var dataString = 'id='+id+'&wfm_note='+wfm_note+'&type='+type;
          $('.id').val(id);
          $('.type').val(type);
          $('.wfm_note').val("");
