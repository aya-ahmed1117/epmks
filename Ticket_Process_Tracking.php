...
<div class="col-md-8">
       ...
       <samp><p style="background-color:#55608f;font-weight:bold;font-size:16px;color:white;padding:2%;">Here you can search for tickets that you have worked on it to see if you follow the process or not</p></samp>
    </aside>
  </div>
</center>
...
<input  name="username" readonly="true"
value='<?php if($s_username != '') echo $s_username;?>'class="form-control">
...
Iif(In_ is null,'No Process','Process') [Process_or_Not]
 ...
 where  cast([OPERATION TIME] as date) BETWEEN '$mydate' AND '$mydate2' and [login name] = '$s_username'
...
$rows  ='<tr>';
  $rows .='<td class="hovers" style="text-align: -webkit-center;border: 1px solid lightgray;">'.$output["RequestID"].'</td>';
  $rows .='<td class="hovers" style="text-align: -webkit-center;border: 1px solid lightgray;">'.$output["Date"]->format('Y/m/d').'</td>';
  $rows .='<td class="hovers" style="text-align: -webkit-center;border: 1px solid lightgray;">'.$output["login name"].'</td>';
  

  if($output["Process_or_Not"] == 'Process' ){
    $rows .='<td class="hovers" style="text-align: -webkit-center;border: 1px solid lightgray;background-color:#4CAF50; color:white;">'.$output["Process_or_Not"].'</td>';
  }else{
  $rows .='<td class="hovers" style="text-align: -webkit-center;border: 1px solid lightgray;color:red;">'.$output["Process_or_Not"].'</td>';}
