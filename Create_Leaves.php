...
if(isset($_POST['send'])){
  ...
  if(empty($_POST["type"])){
    ...
    echo '<script>alert("Please Select type name to proceed");   </script>';
  }
...
if($type == 'Annual Leave'){
      ...
      $outs = sqlsrv_query($con1,"SELECT 
   p.[UserName] ,
     (case when (DATEDIFF(DAY, [Hiring_Date], '$adate')
             >= 180) then 'Eligible'
         else 'Not Eligible' end ) as result
  FROM [Employess_DB].[dbo].[tbl_Personal_info] p
  left join Annuel_cal on Annuel_cal.Username = p.UserName 
  where [Employee_Status] <>'Resigned' and 
  [Employee_Type]='OutSource' 
  and p.USERname ='$s_username'");
 

    
 $result_outs = sqlsrv_fetch_array($outs);
    //$out_user=$result_outs['username'];
    $results= $result_outs['result'];


  //start if outsource < 6 month from hiring 

 if ($results == 'Not Eligible'){
  echo '<script>
    swal({
    title: "You are not eligible to submit a leave request until you have completed your first six months. Thank you.",
...
//php -> output data from mysqli
  ...
  <br/>
      <div class="content">
         Your Operation Couldn`t be completed because your allowed annual days is : '.$Annuel_days.' 
      </div>
    </div>
  </div>
...
<script type="text/javascript">
 ...
 document.addEventListener("DOMContentLoaded", function () {
    // click on  deleteMeetingClose
    document.querySelector(".deleteMeetingClose").addEventListener("click", function () {
      // disapled message
...
<script type="text/javascript">
  ...
  $("#overlay, .deleteMeetingCancel, .deleteMeetingClose").click(function () {
    document.getElementById("message").style.display = "none".then(function() {
            window.location.href=window.location.href
                            });
});*/
</script>';
...
//
if($type == 'unplanned') {
    // Get current unplanned leave
    $unp_query = sqlsrv_query($con, "
        SELECT SUM([count]) AS countUnplanned 
        FROM leaves 
        WHERE [username] = '$s_username' 
          AND YEAR([adate]) = $CurrentYear 
          AND YEAR([bdate]) = $CurrentYear 
          AND [type] = 'unplanned' 
          AND (
              status IN ('PENDING', 'pending', 'E-workforce and senior approve') 
              OR status LIKE '% approve'
          )
    ");

    $countUnplanned = 0;
    if($output_query = sqlsrv_fetch_array($unp_query)) {
        $countUnplanned = (int)$output_query["countUnplanned"];
    }

    // Check if over limit
    if(($countUnplanned + $interval > 6)) {
        echo '<div id="messagess1" class="overlay">
        <div class="popup">
            <h2> Sorry '.$s_username.'</h2>
            <span class="deleteMeetingClose">&times;</span>
            <br/>
            <div class="content">
                You have exceeded the 6 days unplanned leave.
            </div>
        </div>
        </div>
        <script>
            // Show message
            document.getElementById("messagess1").style.display = "block";
    ...
    elseif($interval <= 0) {
        ...
        <div class="popup">
            ...
            <br/>
            <div class="content">
                Your Operation Couldn`t be completed because your request is invalid or exceeds the limit.
            </div>
        </div>
        </div>
        <script>
            document.getElementById("messagesss1").style.display = "block";
        ...
        // Insert unplanned leave
        $insert = sqlsrv_query($con, "
            INSERT INTO leaves 
            ([username], [engineer_id], [adate], [bdate], [starttime], [endtime], [type], [notes], [count], [status], [creation_time]) 
            VALUES 
            ('$s_username', '$engineer_id', '$adate', '$bdate', '$starttime', '$endtime', '$type', '$notes', '$interval', '$status', '$sqltime')
        ");
...
&& $type !== 'Paternity Leave' && $type !== 'Unpaid Leave' && $type !== 'unplanned' && $type !== "Maternity Leave" && $interval > 0  ){
...
if($diffMinutes < 15 ){
  echo '<div id="messagesss4" class="overlay">
  <div class="popup">
    <h2> Attention '.$s_username.'</h2>
    <span class="deleteMeetingClose">&times;</span>
<br/>
    <div class="content">
       Wornning:Permission must be > 15 M
    </div>
  </div>
</div>
<script type="text/javascript">
...
<?php 
$s_username = $_SESSION['username'];
$first_query = sqlsrv_query( $con ,"SELECT * FROM leaves WHERE username ='$s_username'
...
function compare()
{
  debugger;
    ...
    if( (startTime > endTime) && (ptype == "Permission" || ptype == "Official Mission" ) ) {
     ...
     if(ptype == ''){
      alert("Please Select type ya bnty");  
    }
