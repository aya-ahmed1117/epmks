...
//php -> output data from mysqli
  ...
  $outputs_query = sqlsrv_fetch_array($newannual);
   echo $Annuel_days = $outputs_query['Annuel_days'];
      

    $totalss = $countLeaves + $interval  > $Annuel_days;
   
    echo $countLeaves . '<br>a';
    echo $interval . '<br>b';
    echo $Annuel_days . '<br>c';
    echo $totalss .'<br>d';

    echo   $total =($countLeaves+$interval);
...
';
  ...
  elseif(($interval > 0)&& ($_SESSION['username'] !== 'x_test'))
  {
    //sqlsrv_query( $con ,
    $insert_query = "INSERT INTO leaves ([username] , [engineer_id] , [adate] , [bdate] ,[starttime] , [endtime] , [type] , [notes] , [count] , [status] , [creation_time]) VALUES ('$s_username','$engineer_id' , '$adate' , '$bdate' , '$starttime' , '$endtime' , '$type', '$notes' , '$interval' , '$status' , '$sqltime' )";//);
    $total =($countLeaves+$interval);
